export function graficaDiferenciasBarcelona(data) {
  ...
  const filters = d3.select("#filters-diferencias-Barcelona");

  // Filtro los datos para quedarme solo con Barcelona
  const filteredBarcelona = data.filter(d => d.Ciudad === "Barcelona Capital");
  ...
  const barriosUnicos = [...new Set(filteredBarcelona.map(d => d.Barrio))].sort();
  ...
  function updateChart() {
    ...
    svg.append("text")
      ...
      .text("Precio €/m² por barrio y características en Barcelona");
