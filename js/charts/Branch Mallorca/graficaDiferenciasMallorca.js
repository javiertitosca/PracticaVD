export function graficaDiferenciasMallorca(data) {
  ...
  const filters = d3.select("#filters-diferencias-Mallorca");

  // Filtro los datos para quedarme solo con Mallorca
  const filteredMallorca = data.filter(d => d.Ciudad === "Mallorca");
  ...
  const barriosUnicos = [...new Set(filteredMallorca.map(d => d.Barrio))].sort();
  ...
  function updateChart() {
    ...
    svg.append("text")
      ...
      .text("Precio €/m² por barrio y características en Mallorca");
